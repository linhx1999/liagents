...
class TestMessageInit:
    ...
    def test_init_with_metadata(self):
        ...
        msg = Message(role="assistant", content="回复", metadata=custom_metadata)
        assert msg.metadata == custom_metadata
        assert "timestamp" not in msg.metadata  # 不会被覆盖

    def test_init_with_custom_timestamp(self):
        """测试带自定义时间戳的初始化"""
        custom_time = datetime(2024, 1, 1, 12, 0, 0)
        msg = Message(role="user", content="测试", timestamp=custom_time)
        assert msg.metadata["timestamp"] == custom_time
...
class TestMessageToDict:
    ...
    def test_to_dict_excludes_metadata(self):
        """测试转换后不包含 metadata"""
        msg = Message(role="assistant", content="回复", metadata={"extra": "data"})
        result = msg.to_dict()
...
class TestMessageEquality:
    ...
    def test_equal_messages(self):
        """测试相等的消息"""
        msg1 = Message(role="user", content="测试")
        msg2 = Message(role="user", content="测试")
    ...
    def test_different_roles(self):
        """测试不同角色的消息"""
        msg1 = Message(role="user", content="测试")
        msg2 = Message(role="assistant", content="测试")
        assert msg1.role != msg2.role

    def test_different_content(self):
        """测试不同内容的消息"""
        msg1 = Message(role="user", content="消息1")
        msg2 = Message(role="user", content="消息2")
        assert msg1.content != msg2.content
...
class TestMessageEdgeCases:
    ...
    def test_special_characters_in_content(self):
        ...
        msg = Message(role="user", content=special_content)
