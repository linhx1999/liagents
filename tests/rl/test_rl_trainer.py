...
import json
from builtins import __import__ as __import_original__
...
def mock_dataset():
    ...
    dataset = Dataset.from_dict(
        {"prompt": ["Question 1", "Question 2"], "completion": ["Answer 1", "Answer 2"]}
    )
    return dataset
...
def rl_trainer(mock_tokenizer):
    ...
    with patch(
        ...
        return RLTrainer(
            model_name_or_path="Qwen/Qwen3-0.6B", output_dir="./test_outputs"
        )
...
class TestRLTrainerInit:
    ...
    def test_init_basic(self, mock_tokenizer_fn, mock_tokenizer):
        ...
        trainer = RLTrainer(
            model_name_or_path="Qwen/Qwen3-0.6B", output_dir="./outputs"
        )
    ...
    def test_init_custom_model(self, mock_tokenizer_fn, mock_tokenizer):
        ...
        trainer = RLTrainer(
            model_name_or_path="custom/model/path", output_dir="./outputs"
        )
    ...
    def test_init_with_spaces_in_model_name(self, mock_tokenizer_fn, mock_tokenizer):
        ...
        trainer = RLTrainer(
            model_name_or_path="Model With Spaces", output_dir="./outputs"
        )
...
class TestTrainMethod:
    ...
    def test_train_sft_algorithm(self, mock_check_trl, rl_trainer_with_dataset):
        ...
        # Mock _train_sft 方法
        rl_trainer_with_dataset._train_sft = Mock(
            return_value={"status": "success", "algorithm": "SFT"}
        )

        result = rl_trainer_with_dataset.train(
            algorithm="sft", batch_size=8, num_epochs=3
        )
    ...
    @patch("liagents.rl.trainer.check_trl_installation")
    def test_train_parameters_passed_correctly(
        self, mock_check_trl, rl_trainer_with_dataset
    ):
        """测试训练参数正确传递"""
        mock_check_trl.return_value = True
...
class TestOutputDirectory:
    ...
    def test_output_dir_absolute_path(self, mock_tokenizer_fn, mock_tokenizer):
        ...
        trainer = RLTrainer(model_name_or_path="test/model", output_dir="./outputs")
...
class TestEdgeCases:
    ...
    def test_train_with_custom_dataset_and_no_registered(
        self, rl_trainer, mock_dataset
    ):
        """测试只提供自定义数据集（没有注册数据集）"""
        ...
        with patch("liagents.rl.trainer.check_trl_installation", return_value=True):
            ...
            rl_trainer.train(algorithm="sft", custom_dataset=mock_dataset)
